...
func (s *Space) CellServerID() string {
	return s.initCellServerID
}
...
func (s *Space) OnSpaceInit() error {
	return nil
}
...
// OnCellPartCreated cell部分创建好了
func (s *Space) OnCellPartCreated() error {
	return nil
}

// PrepareCellData 创建cell时传过去的数据
func (s *Space) PrepareCellData() map[string]string {
	return map[string]string{}
}

// OnEntityEnter 实体进入后回调
// func (s *Space) OnEntityEnter(entity *BaseEntity) {}

// // OnEntityLeave 实体离开后回调
// func (s *Space) OnEntityLeave(entity *BaseEntity) {}
...
// func (s *Space) enterZeroRadiusEntities(entities []*BaseEntity) {
// 	t := time.Now()
...
// 		// 统计每种类型的实体分别插入了多少个
// 		if num, ok := stat[e.typeDesc.name]; ok {
// 			stat[e.typeDesc.name] = num + 1
// 		} else {
// 			stat[e.typeDesc.name] = 1
// 		}
// 	}
...
// func (s *Space) _beforeEnter(e *BaseEntity, pos *math32.Vector3) {
// 	// 注意，这里的代码 时序很重要
// 	e.space = s
// 	s.entities.Add(e)
...
// func (s *Space) _afterEnter(e *BaseEntity) {
// 	s.I.OnEntityEnter(e)
// 	e.I.AfterEnterSpace()
...
// // 通知实体已经进入到场景
// func (s *Space) notifyEntityEnterSpaceResult(e *BaseEntity) {
// 	// TODO 这里只通知玩家进入场景，可以考虑用依赖反转，给具体实体控制是否需要通知回业务 app
// 	if e.TypName() != metapart.TypNamePlayer {
// 		return
// 	}
// 	msg := &pb.EnterSpaceResultNotify{
// 		Ok:            true,
// 		ErrCode:       "",
// 		ErrMsg:        "",
// 		SpaceID:       s.ID,
...
// func (s *Space) enter(entity *BaseEntity, pos math32.Vector3) {
...
// func (s *Space) enterRef(entity *BaseEntity, pos math32.Vector3, ref *BaseEntity) {
// 	s._beforeEnter(entity, &pos)
...
// func (s *Space) leave(entity *BaseEntity) {
...
// func (s *Space) move(entity *BaseEntity, newPos *math32.Vector3) {
// 	if s.aoiMgr == nil {
...
// // entityID 可以为空字符串
// func (s *Space) CreateEntity(typName string, entityID string, attrs *attr.StrMap, pos math32.Vector3) *BaseEntity {
// 	e := CreateEntity(
// 		typName,
// 		entityID,
// 		attrs,
...
// func (s *Space) EnterBatchEntities(entities []*BaseEntity) {
// 	s.enterZeroRadiusEntities(entities)
// }

// // EnterEntityWithRef 进入单个实体，只允许初始化地图数据时调用
// func (s *Space) EnterEntityWithRef(entity *BaseEntity, pos math32.Vector3, ref *BaseEntity) {
// 	s.enterRef(entity, pos, ref)
// }

// // EnterSingleEntity 进入单个实体
// func (s *Space) EnterSingleEntity(entity *BaseEntity, pos math32.Vector3) {
// 	s.enter(entity, pos)
// }

// // LoadEntity laod entity in this space
// func (s *Space) LoadEntity(typName string, entityID string) (*BaseEntity, error) {
...
func (s *Space) GetEntityByType(typName string) BaseMap {
	entities := GetEntitiesByType(typName)
	return entities.Filter(func(e *Entity) bool {
		return e.TypName() == typName && s.entities.Contains(e)
	})
}

// cellPartCreated cell部分创建好了
func (s *Space) cellPartCreated() error {
	// 先回调到业务方的
	// 场景的cell部分创建好后，回调回去，业务可以开始加载实体并加入到这个场景里面去
	err := s.I.OnCellPartCreated()
	if err != nil {
		return err
	}
