...
// ZeroVec3  x,y,z 全部为0的
var ZeroVec3 = Vector3{}

// InvalidVec3 x,y,z 全部为最大float32 的 vec3
var InvalidVec3 = Vector3{X: math.MaxFloat32, Y: math.MaxFloat32, Z: math.MaxFloat32}
...
// FromString from x,y,z format to vector3
func (v *Vector3) FromString(str string) *Vector3 {
	components := strings.Split(str, ",")
	l := len(components)
	if l >= 1 {
		f, err := strconv.ParseFloat(components[0], 10)
		if err == nil {
			v.X = float32(f)
		}
	}
	if l >= 2 {
		f, err := strconv.ParseFloat(components[1], 10)
		if err == nil {
			v.Y = float32(f)
		}
	}
	if l >= 3 {
		f, err := strconv.ParseFloat(components[2], 10)
		if err == nil {
			v.Z = float32(f)
		}
	}
...
// ToVec2 convert to vector2
func (v *Vector3) ToVec2() Vector2 {
	return Vector2{
		X: v.X,
		Y: v.Z,
	}
...
//RandomCirclesPoint 范围随机一个点
func (v *Vector3) RandomCirclesPoint(radius float64) {
	angle := rand.Float64() * math.Pi * 2
	v.X += float32(math.Cos(angle) * radius)
	v.Z += float32(math.Sin(angle) * radius)
}

var tmpForMoveTowardDiff = Vector3{}
var tmpForMoveTowardResult = Vector3{}

func (v Vector3) MoveToward(target *Vector3, distance float32) (Vector3, bool) {
	tmpForMoveTowardDiff.X = target.X - v.X
	tmpForMoveTowardDiff.Y = target.Y - v.Y
	tmpForMoveTowardDiff.Z = target.Z - v.Z

	magnitude := tmpForMoveTowardDiff.Magnitude()
	var f float32
	if magnitude <= distance || magnitude == f {
		return *target, true
	}
	tmp := distance / magnitude
	addx := tmpForMoveTowardDiff.X * tmp
	addy := tmpForMoveTowardDiff.Y * tmp
	addz := tmpForMoveTowardDiff.Z * tmp
...
func (v *Vector3) Magnitude() float32 {
	...
	y := v.Y * v.Y
	z := v.Z * v.Z
	return float32(math.Sqrt(float64(x + y + z)))
}
